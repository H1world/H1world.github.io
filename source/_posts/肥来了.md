...
//引入模块
const otherRouter = [
  ...require('../page/router').default, //将版块引入（包含多个子路由配置）
]
<Switch>
  {renderRoutes(otherRouter)}
</Switch>
```
### React遇到的问题
公司有一台元老级别的安卓测试机，android版本4.4。react项目集体GG，经常查找资料发现问题所在，React依赖集合类型 Map 和 Set 在低级浏览器中并不兼容。解决方案：可以引入core-js 或 babel-polyfill。
下载` npm install core-js --save ` 或者 ` npm install babel-polyfill --save `
```js
// 可以在webpack entry内修改添加也可以在入口文件内修改
// core-js 在 entry内的配置
